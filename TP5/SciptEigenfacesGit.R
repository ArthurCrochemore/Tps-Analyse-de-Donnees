...
# Methode qui a partir d'une image va quantifier les niveaux de gris entre 0 et 1 et va les stocker dans un fichier csv
# Cette methode est utilisée pour la creation du fichier olivetti_X.csv
transformImage <- function(imagePath){
  img <- readJPEG(imagePath) %>% # Load image
    as.vector() %>% # Convert image into a vector
    data.frame() # Convert vector into a data frame
  write.csv(img, file = "test.csv", append = TRUE, row.names = FALSE, col.names = FALSE) # Write data frame into a csv file
}

#pour toutes les images dans le dossier yalefaces, on les transforme en fichier csv

list.files() %>% # List all files in the current directory
  grep("jpg", ., value=TRUE) %>% # Select only files with the extension jpg
  paste0("yalefaces/", .) %>% # Add the path to the files
  lapply(transformImage) # Apply the function transformImage to all files with the extension pgm
